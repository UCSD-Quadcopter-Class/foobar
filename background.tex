...
\cfunc{RefCell::borrow\_mut} and \cfunc{RefCell::borrow}, respectively.  These
...
\item \csym{RefCell<T>} (described above) is a wrapper type holding an object of type \csym{T} that
...
Rust provides optional values in form of \csym{Option<T>} that can be either \csym{Some(data)} (where \csym{data} is of type \csym{T}), or \csym{None}.  Its most common use to allow for null pointers: \csym{Box<T>} is always a valid pointer to allocated memory, but \csym{Option<Box<T{>}>} can either be \csym{None} or a \csym{Box<T>}.
...
implement by default but can opt out of.  For instance, all primitive types
implement the \csym{Copy} trait so they can be passed by value to functions.
We say those types ``are \csym{Copy}''.  \csym{Box<T>} does not implement
\csym{Copy} since copying it would result in two pointers to the same data.
So, we say \csym{Box<T>} is \csym{!Copy}.

Another auto trait, \csym{Send} labels types that can pass from one thread
to another.
...
In response to serious errors, programs can call \cfunc{panic!} to end the
