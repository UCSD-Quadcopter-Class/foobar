...
\steve{Wrong example. Copy is not an auto trait. It is a special tairt that
the compiler decides what type may or may-not have. `Send' is a good example
which is an auto trait for all types except the ones that opt out for, such
as Rc.}
