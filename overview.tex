...
\boldparagraph{Pointers from volatile memory into a pool} VWeak is the only way store a pointer to PMEM in volatile memory. The object a
...
and \csym{Parc} \csym{!Send} to prevent their  capture by or transmission to another thread.  \csym{VWeak} remains Send, so it can be used to transmit persistent state between threads.
