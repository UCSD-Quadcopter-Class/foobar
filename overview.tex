...
hold at the boundaries.  For instance, \this{} guarantees that leaks in acyclic structures are not
...
Reference and raw pointers, and types that refer to state external to the program (e.g., file handles) are \csym{!PSafe}. Having a \csym{!PSafe} field also makes a type \csym{!PSafe}. As a result, \csym{Box}, \csym{Rc}, \csym{Arc}, \csym{Mutex}, and the other types in Rust containing raw pointers are \csym{!PSafe}.
...
argument, \csym{j}, which is a reference to a \emph{journal object} that holds information about the current transaction.  Variable
...
\begin{discuss}
  ...
  \csym{Journal<P>} is stranded, so it cannot escape the transaction.
...
{ \centering
  ...
  \label{tab:types}
  \begin{tabular}{|l|l|p{3.6in}|}\hline
...
\csym{VWeak} %is a pointer from volatile memory into a pool.
is the only way to hold a volatile pointer to persistent data.
% \steve{The motivation for Vweak is not clear.  Is it the case that Vweak is the only way to have a pointer from DRAM into PMEM?} \morteza{Yes, it is. I'm not sure what should we add?}
It is ``weak'' in
...
\boldparagraph{Pointers from volatile memory into a pool} \csym{VWeak} is the only way store a pointer to PMEM in volatile memory. The object a
\csym{VWeak} refers to can disappear if the last \csym{Prc}/\csym{Parc} refering to the
...
closed, and a pool will not close while a transaction is in progress.

Second, consider a reference (other than a \csym{VMWeak}), \csym{A}, that exists
outside a transaction.  Rust's borrow checker requires that a chain of in-scope
references exists from the root of the pool to \csym{A}.  This includes the root pointer,
...
\begin{goaltrue}{goal:no-races}
  ...
  invariant, the \csym{Mutex} type, and marker types to restrict data movement between threads.
...
a method, \cfunc{pclone}, to create a new reference to the shared
data and increment the reference count.  The reference counts are persistent and must be logged, so \cfunc{pclone} takes a journal object argument.
...
To prevent this, \this{} makes \csym{Pbox}
and \csym{Parc} \csym{!Send} to prevent their  capture by or transmission to another thread.  \csym{VWeak} remains \csym{Send}, so it can be used to transmit persistent state between threads.
...
against higher level errors (e.g., whether the algorithm in our example correctly appends to a linked list).
...
More permissive libraries allow this, but such code would not compile under \this{}.  Ideally, \this{} could grow to include
...
something that was helpful in ensuring safety and was implementable in Rust.
...
type system.  Recreating \this{}'s approach in C++ would require
...
goals using a more idiomatic C++ approach.
