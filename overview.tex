...
\This{} provides an additional guarantee that is necessary for persistent memory: It ensures that, by the end of the allocating transaction, the newly allocated persistent memory is reachable from some previous allocated persistent memory.  This ensures that crashes (which
invalidate all volatile pointers) do not leak volatile memory.

\begin{goaltrue}{goal:no-memory-leaks}

\This{} adds three mechanisms Rust's reference counting mechanism to prevent memory leaks in acyclic persistent data structures.
First, it extends Rust's reference counting mechanism to persistent memory:
