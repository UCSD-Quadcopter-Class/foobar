...
\reffig{fig:scal} confirms that the performance scales with thread count.  The bottleneck in this experiment is the contention the shared data structures the threads use to communicate.
...
\begin{figure}
    ...
    The \csym{wordcount} benchmark uses one producer thread that buffers characters from files and multiple consumer threads
