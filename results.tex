...
\begin{table}
  \center
  \footnotesize
  \caption{\textbf{\This's basic operation latency and maximum latency} for durability and safety support measured on Intel's Optane DC and Battery-Backed DRAM, with 50K operations per test.}
...
\reftab{tab:microbench} reports the latency of basic operations along with the maximum latency of the fastest 99\% of the operations measured on the platform described in \refsec{sec:res:platform}. To evaluate the impact of storage technology, we measure these operations on both Optane DC persistent memory and DRAM. Some of the items have a very small average latency, so we measure a batch of operations and calculate the average. This prevents us from calculating the maximum latency (of 99\%) for those operations.
