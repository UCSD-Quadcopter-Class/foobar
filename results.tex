...
\reftab{tab:microbench} reports the latency of basic operations along with the standard deviation measured on the platform described in \refsec{sec:res:platform}. To evaluate the impact of storage technology, we measure these operations on both Optane DC persistent memory and DRAM. ``STD'' is the standard deviation. Some of the items have a very small average latency, so we measure a batch of operations and calculate the average. This prevents us from calculating the standard deviation for those operations.

To measure dereferencing operations, we use a \csym{Pbox<i32>}. Dereferencing a persistent pointer involves address translation and memory indirection. The Rust compiler uses CPU registers to cache the base and offset addresses. As a result, the dereferencing operation performs less than 1~ns, for both read and write. However, writing for the first time requires logging which takes around 470~ns.
...
The Wordcount benchmark measures scalability.  It collects character strings from multiple text, and measures the frequency of each word in a set of threads (consumers).
\This{} provides
a separate allocator and journal object for every thread to allow concurrency. %concurrent allocation.
\reffig{fig:scal} shows that the performance scales with thread count.
Since Wordcount is embarrassingly parallel, this shows that Corundum does not limit scalability.
