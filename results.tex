...
and sometimes significantly faster. Although in some cases it is not the fastest,
we should note that it is the only library that provides the highest level of safety.
...
When the allocator cannot find a free block in its free lists, it deliberates the task to another allocator which is protected by a lock (not occurred in this experiment).
